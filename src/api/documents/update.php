...
$today = date('Y-m-d');
if ($date_facture && $date_facture > $today) {
    echo json_encode(['status' => 'error', 'message' => "La date du document ne peut pas être dans le futur"]);
    exit;
}
if ($date_echeance && $date_echeance > $today) {
    echo json_encode(['status' => 'error', 'message' => "La date ne peut pas être dans le futur"]);
    exit;
}
