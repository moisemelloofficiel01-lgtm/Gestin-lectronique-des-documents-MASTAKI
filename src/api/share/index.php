...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'share_document') {
        ...
        $sharedWithArr = is_array($data['shared_with']) ? $data['shared_with'] : [$data['shared_with']];

        $inserted = 0;
        $skipped = 0;
        foreach ($sharedWithArr as $uid) {
            $sharedWith = (int)$uid;
        ...
        if ($inserted > 0) {
            echo json_encode(["status" => "success", "message" => "$inserted utilisateur(s) ajouté(s)" . ($skipped > 0 ? " ($skipped déjà partagé(s))" : "")]);
        } else {
            echo json_encode(["status" => "error", "message" => "Aucun nouveau partage effectué ($skipped déjà partagé(s))"]);
        }
        exit;
    }
    ...
    if ($action === 'share_category') {
        ...
        if ($inserted > 0) {
            ...
            echo json_encode(["status" => "error", "message" => "Aucun nouveau partage ($skipped déjà partagé(s))"]);
        }
        exit;
    }
